/******************************************************************************
MonarchLW.cpp
Library for interfacing with the Project Monarch long wave pyranometer 
...
uint8_t MonarchLW::begin() 
{
	Wire.begin();
  WriteWord_LE(ADC_ADR, ADC_CONF, ADC_CONF_MASK); //Set to single shot mode with 
}

float MonarchLW::GetThermo()
{
  return GetADC(ADC1)*(0.125); //Convert to voltage [mV]  //DEBUG!
}

float MonarchLW::GetThermoC(uint8_t Accuracy)  //0 for low accuracy (default), 1 for high accuracy, increases compute time and memory!
{
...
float MonarchLW::TempConvert(float V, float Vcc, float R, float A, float B, float C, float D, float R25)
{
...
float MonarchLW::TempConvert(float V, float Vcc, float R, float Beta, float R25)
{
  // float Rt = (Vcc/V)*R - R;  //Use if thermistor is on TOP side of voltage divider
  float Rt = -R/(1 - (Vcc/V)); //Use if thermistor is on BOTTOM side of voltage divider  
...
uint8_t MonarchLW::WriteWord_LE(uint8_t Adr, uint8_t Command, unsigned int Data)  //Writes value to 16 bit register
{
...
int MonarchLW::ReadWord_LE(uint8_t Adr, uint8_t Command)  //Send command value, returns entire 16 bit word
{
...
uint8_t MonarchLW::SendCommand(uint8_t Adr, uint8_t Command)
{
    Wire.beginTransmission(Adr);
    Wire.write(Command);
    return Wire.endTransmission(); 
